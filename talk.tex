...
\end{center}
\vspace{-10pt}
\src{Science 2023 SM Tables S1 to S4 at $N=128$, 1\,GHz; DCPS: digital-control phase shifters. Envise: 78\,W + 1.6\,W optical at 65.5\,TOPS, 0.25\,W in the encode path (Nature 2025 SI). 4-bit: ADC, DAC $\propto 2^{b}$; digital op $\div 3$; inference light $\propto 4^{b}$, training light at the tap-SNR floor.}\\[0pt]
{\scriptsize\textbf{Reading the chart}: at 8 bits the input DAC is the largest term. At 4 bits the TIAs dominate: inference sits 6$\times$ under the 4-bit digital line, training 1.3$\times$. Envise's optics and encode are 28\,fJ/op; the other 1.19\,pJ/op is the digital chip.}
\note{Assumptions behind the 4-bit projection: ADC and DAC energy scale as 2 to the bits (Walden); the Nature SI's thermal-limited 4 to the bits would be 16 times more optimistic. Digital op divided by 3 is the B200 FP4 to H100 INT8 ratio at the wall. Inference optical power scales as 4 to the bits from shot noise; training optical power stays at the 8-bit value because the taps need 0.5 microwatts to see the gradient. Best-of replaces the 2023 modulator and input DAC with Envise's measured encode path. Three things to say. One: the 2023 tables already predicted that conversions would dominate, and Envise confirmed it with a factor of 40 between the tensor core and the package. Two: 4-bit inference is where the optics win on paper, because ADC and DAC energy collapse and the optical power needed for 4-bit SNR is tiny. Three: training does not get the same gift, because the tap photodiodes need enough light to see a shrinking gradient, and the TIA per phase shifter is bit-independent. That is the honest gap between inference and training accelerators.}
\end{frame}
