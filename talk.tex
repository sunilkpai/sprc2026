...
\end{center}
\vspace{-10pt}
\src{2023 SM Tables S1 to S4, $N=128$, 1\,GHz, segmented phase shifters for inputs and weights (no DACs). Envise: 78\,W + 1.6\,W optical at 65.5\,TOPS, 0.25\,W encode incl.\ weight DACs. 4-bit: ADC $\propto 2^{b}$, segments $\propto b$, digital op $\div 3$, inference light $\propto 4^{b}$, training light at the tap-SNR floor.}\\[0pt]
{\scriptsize\textbf{Reading the chart}: segmented weights put the encode at 0.1\,fJ/op, so the ADC dominates at 8 bits and the TIA at 4. Inference at 4 bits: 7$\times$ under the 4-bit digital line; training 1.4$\times$ ($M=16$), 2.7$\times$ ($M=64$). The price: $N^{2}b$ contacts, 65k at 4 bits, a bump array per weight.}
