...
DEFAULT = dict(E_mod=E_mod, E_sw=E_sw, E_TIA=E_TIA, E_DAC=E_DAC, E_ADC=E_ADC,
               E_OP=E_OP, E_mode=E_mode)


def components(N, M, digital_control_ps=False, **override):
    """Per-component energy (J) of the in situ MVM and in situ VJP/grad for one
    N x N layer at batch size M, following the Table S4 counts.  Returns two dicts
    keyed by component name.  `override` replaces any Table S1 value."""
    c = dict(DEFAULT); c.update(override)
    e_mod, e_dac = (16 * c["E_mod"], 0.0) if digital_control_ps else (c["E_mod"], c["E_DAC"])
    MN = M * N
    mvm = {"digital I/O prep": 12 * MN * c["E_OP"], "modulator": 2 * MN * e_mod,
           "DAC": 2 * MN * e_dac, "ADC": 4 * MN * c["E_ADC"], "TIA": 4 * MN * c["E_TIA"],
           "optical power": 2 * MN * c["E_mode"]}
    grad = {"digital I/O prep": 36 * MN * c["E_OP"], "modulator": 6 * MN * e_mod,
            "DAC": 6 * MN * e_dac, "ADC": 8 * MN * c["E_ADC"],
            "TIA": 8 * MN * c["E_TIA"] + 4 * N**2 * c["E_TIA"],
            "optical power": 6 * MN * c["E_mode"], "switches": 20 * N**2 * c["E_sw"]}
    return mvm, grad


def energies(N, M, digital_control_ps=False, **override):
    ...
    E_out = (4 * c["E_TIA"] + 4 * c["E_ADC"]) * M * N
    E_grad_upd = (2 * c["E_TIA"] + 10 * c["E_sw"]) * 2 * N**2
    E_MVM_dig = 6 * E_OP_ * M * N**2
    E_ioprep = (6 * E_OP_ + c["E_mode"]) * M * N
    # Table S3 tasks
    ...
    mvm, grad = components(N, M, digital_control_ps, **override)
    assert np.isclose(sum(mvm.values()), E_MVM_alg, rtol=1e-12, atol=0)
    assert np.isclose(sum(grad.values()), E_grad_alg, rtol=1e-12, atol=0)
