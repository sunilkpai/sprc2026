...
| `scripts/attention_share.py` | Per-token MACs per layer for Llama 3 70B, DeepSeek-V3, V3.2, V4 Pro and Kimi K3 from their published configs: active-expert MLP, MLA projections, and the attention core (dense, DSA-sparse with its indexer, KDA linear) at 8k, 128k and 1M context. Feeds the table on the MLP-versus-attention slide. |
